            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-medium text-gray-900">Danh sách Kho hàng</h3>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên Kho</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($warehouses as $warehouse)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $warehouse->address }}, {{ $warehouse->city }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('warehouses.show', $warehouse->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Xem</a>
                                            <a href="{{ route('warehouses.edit', $warehouse->id) }}" class="text-yellow-600 hover:text-yellow-900 mr-3">Sửa</a>
                                            <form action="{{ route('warehouses.destroy', $warehouse->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn có chắc muốn xóa kho hàng này?');">
                                                @csrf
                                                @method('DELETE')
