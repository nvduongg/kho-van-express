<x-app-layout>
                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('orders.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <x-text-input id="order_date" class="block mt-1 w-full" type="date" name="order_date" :value="old('order_date', \Carbon\Carbon::now()->format('Y-m-d'))" required />
                            ...
                            <div>
                                <div class="mb-4">
                                    ...
                                    <x-text-input id="shipping_address" class="block mt-1 w-full" type="text" name="shipping_address" :value="old('shipping_address')" />
                                    <x-input-error :messages="$errors->get('shipping_address')" class="mt-2" />
                                </div>
                                <div class="mb-4">
                                    <x-input-label for="shipping_city" :value="__('Thành phố')" />
                                    <x-text-input id="shipping_city" class="block mt-1 w-full" type="text" name="shipping_city" :value="old('shipping_city')" />
                                    <x-input-error :messages="$errors->get('shipping_city')" class="mt-2" />
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="mb-4">
                                        <x-input-label for="shipping_state" :value="__('Tỉnh/Bang')" />
                                        <x-text-input id="shipping_state" class="block mt-1 w-full" type="text" name="shipping_state" :value="old('shipping_state')" />
                                        <x-input-error :messages="$errors->get('shipping_state')" class="mt-2" />
                                    </div>
                                    <div class="mb-4">
                                        <x-input-label for="shipping_zip_code" :value="__('Mã bưu chính')" />
                                        <x-text-input id="shipping_zip_code" class="block mt-1 w-full" type="text" name="shipping_zip_code" :value="old('shipping_zip_code')" />
                        ...
                        <div id="order-items-container">
                            @if(old('product_ids'))
                                @foreach(old('product_ids') as $index => $old_product_id)
                                    @php
                                        $product = $products->firstWhere('id', $old_product_id);
                                    @endphp
                                    <div class="flex items-center gap-4 mb-4 order-item-row p-4 border border-gray-200 rounded-md bg-gray-50">
                                        <div class="flex-grow">
                                            ...
                                            <select name="product_ids[]" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 product-select" required>
                                                ...
                                                @foreach($products as $prod)
                                                    <option value="{{ $prod->id }}" data-price="{{ $prod->price }}" {{ $prod->id == $old_product_id ? 'selected' : '' }}>
                                        ...
                                        <div class="w-24">
                                            ...
                                            <x-text-input type="number" name="quantities[]" class="block mt-1 w-full quantity-input" min="1" value="{{ old('quantities.' . $index) }}" required />
                                            <x-input-error :messages="$errors->get('quantities.' . $index)" class="mt-2" />
                                        </div>
                                        <div class="w-32">
                                            <x-input-label :value="__('Tổng cộng')" />
                                            <span class="block mt-1 w-full py-2 px-3 text-gray-700 total-price">{{ number_format($product->price * old('quantities.' . $index), 0, ',', '.') }} VNĐ</span>
                                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Add event listeners to existing rows (in case of old input)
            ...
            addItemButton.addEventListener('click', function () {
                ...
                newRow.innerHTML = `
                    <div class="flex-grow">
                        ...
                        <select name="product_ids[]" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 product-select" required>
                            <option value="">Chọn sản phẩm</option>
                            ${productOptions.map(product => `<option value="${product.id}" data-price="${product.price}">${product.name} - ${product.price.toLocaleString('vi-VN')} VNĐ</option>`).join('')}
                        </select>
                    </div>
                    <div class="w-24">
                        <x-input-label :value="__('Số lượng')" />
